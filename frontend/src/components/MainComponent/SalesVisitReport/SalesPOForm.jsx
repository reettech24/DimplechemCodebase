...
const SalePOForm = () => {
  ...
  const handleExportData = () => {
    const data = salesActivityData?.data.map((deal, index) => ({
      ...
      "No. of Visits": deal.communications?.[0]?.followup_summary
        ? (deal.communications[0].followup_summary.match(/^\d+\./gm) || [])
            .length
        : "-",
      "Total Hrs Spend": deal.communications?.[0]?.total_hrs_spent || "-",
      "Approx Area (sq.m)": deal.project_name || "-",
      "Total Product Qty. (Kg)": deal.total_material_qty || "-",
      "Total Potential Amount": deal.approx_business || "-",
      "Types of Documents": "-", // Placeholder
      "Last Visit Date": deal.assign_date
        ? new Date(deal.assign_date).toLocaleDateString()
        : "-",
      "Last Visit Remark": deal.communications?.[0]?.followup_summary || "-",
      "Next Visit Plan": deal.next_followup
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Sales Activity Report");
    ...
    saveAs(blob, "Sales_Activity_Report.xlsx");
  };
