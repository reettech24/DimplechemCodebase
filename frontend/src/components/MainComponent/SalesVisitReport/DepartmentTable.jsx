        ...
        <tbody>
          {finalizeDealsData.map((deal, index) => (
            <tr key={deal.id || index}>
              ...
              <td className="px-4 py-2 text-newtextdata text-center">
                {deal.communications[0]?.total_hrs_spent ?? "-"}
              </td>{" "}
              ...
              <td className="px-4 py-2 text-newtextdata text-center">
                {deal.total_material_qty ?? "-"}
              </td>
              <td className="px-4 py-2 text-newtextdata text-center">
                {deal.approx_business ?? "-"}
              </td>
              ...
              <td className="relative group cursor-pointer px-4 py-2 text-newtextdata whitespace-normal">
                {(() => {
                  const summary =
                    deal.communications?.[0]?.followup_summary || "";
                ...
                {(() => {
                  ...
                  const words = summary.split(" ");
                  const tooltipWidth =
                    words.length > 5 ? "w-[300px]" : "w-auto";

                  return (
                    <div
                      className={`absolute z-10 hidden group-hover:block bg-white text-gray-800 text-sm rounded-md px-3 py-1 top-10 left-1/2 -translate-x-1/2 whitespace-normal ${tooltipWidth} shadow-lg text-justify`}
                    >
                      {summary}
                    </div>
                  );
