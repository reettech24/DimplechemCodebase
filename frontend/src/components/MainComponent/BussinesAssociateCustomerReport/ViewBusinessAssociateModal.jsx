...
const ViewBusinessAssociateModal = ({ setViewBAModalOpen,selectedBusinessAssocitae }) => {
      ...
      <div className="fixed inset-0 p-2 bg-black/50 flex items-center justify-center z-50">
        <div className="bg-white w-full w-full md:w-fit pt-0 pb-4 rounded-[6px] flex flex-col">
          <h2 className="text-white text-[20px] font-poopins mb-2 px-0 py-2 text-center bg-bgDataNew rounded-t-[5px]">
            Business Associate with customer Detail
          </h2>
  ...
  label="Customer(s)"
  value={
    selectedBusinessAssocitae?.customers?.length > 0
      ? selectedBusinessAssocitae.customers.map((cust, index) => (
          <div key={cust.id}>
            {index + 1}. {cust.company_name}
          </div>
        ))
      : "-"
  }
